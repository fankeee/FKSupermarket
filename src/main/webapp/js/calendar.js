...
(function($, window, document, undefined) {
	...
	Calendar.prototype = {
		showHoverInfo: function(obj) { // hover 时显示当天信息
			...
			var offset_t = $(obj).offset().top + (this.$calendar_today.height() - $(obj).height()) / 2 - 160;
			var offset_l = $(obj).offset().left + $(obj).width();
			var changeStr1 = _dateStr.substr(0, 4) + '-' + _dateStr.substr(4, 2) + '-' + _dateStr.substring(6);
			var changeStr = _dateStr.substr(4, 2) + '-' + _dateStr.substring(6);
			var _week = changingStr(changeStr1).getDay();
			var _weekStr = '';
